...
bool Store::is_data_buffer_ready(short& data_tag)
{
	...
	for (vector<SeBuffer>::size_type i = 0; i < se_table_buffer.size(); i++)
	{
		if (se_table_buffer[i].tag == data_tag & se_table_buffer[i].valid1 == 0)//tag match and valid1 == 0    valid2?
		{
			interspace = 1;
			break;
		}
		else if (se_table_buffer[i].tag != data_tag & se_table_buffer[i].valid1 == 0 & se_table_buffer[i].tag == 0)   //valid2?
		{
			interspace = 1;
			break;
		}
